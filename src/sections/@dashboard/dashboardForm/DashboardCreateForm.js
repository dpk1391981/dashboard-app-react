...
const DashboardCreateForm = ({
  ...
  const handleColorChange = (color) => {
    ...
    dispatch(change("DashboardCreateForm", "color_code", colorCode["hex"]));
  };
  ...
  const resetForm = () => {
    dispatch(reset("DashboardCreateForm"));
  };
  ...
  const onSubmit = async (data) => {
    ...
    const saveCreate = await createDashboard(payload, user["_id"]);
    console.log(`saveCreatesaveCreate`);
    console.log(saveCreate);
    if (saveCreate) {
      resetForm();
      navigate("/dashboard/view/" + saveCreate["data"]["_id"], { replace: true });
    }
  };

  const handleSwitchClick = () => {
    if (!isCustomColor) {
      dispatch(change("DashboardCreateForm", "color_code", null));
    }
  };
            ...
            <Grid item>
              <Typography variant='h4' sx={{ mb: 5 }}>
                <div className='create-new-dashboard'>
                  Create New Dashboard <span className='edit-mode'>Create Mode</span>
                </div>
...
const mapStateToProps = (state) => ({
  ...
  form: state.form.DashboardCreateForm.values,
...
export default reduxForm({
  ...
  validate: validation,
})(connect(mapStateToProps, { createDashboard })(DashboardCreateForm));
